...
| `secondary_name` | string / object | - | selects what name will show when `- name` is used for `secondary` when a secondary object config is used. If not set it defaults to `name`. Both string and flexible name object are supported. See [flexible name object](#flexible-name-object). |
